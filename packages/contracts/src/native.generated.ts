...
export const updatePresentationSchema = z.enum(["row","sheet"]);
export const updateStatusSchema = z.discriminatedUnion("status", [z.object({ status: z.literal("unavailable") }).strict(), z.object({ status: z.literal("idle") }).strict(), z.object({ status: z.literal("checking") }).strict(), z.object({ status: z.literal("upToDate") }).strict(), z.object({ version: z.string().min(1).max(64), presentation: updatePresentationSchema, status: z.literal("available") }).strict(), z.object({ version: z.string().min(1).max(64), progressPercent: z.number().int().nonnegative().max(100).nullable().optional(), status: z.literal("downloading") }).strict(), z.object({ version: z.string().min(1).max(64), status: z.literal("installing") }).strict(), z.object({ version: z.string().min(1).max(64).nullable().optional(), failure: updateFailureSchema, status: z.literal("failed") }).strict()]);
