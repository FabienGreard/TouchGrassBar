...
function matchesKey(candidate: StoredDoomerboardKey, expected: DoomerboardKey) {
...
async function inspectDoomerboardOnce(ctx: ActionCtx): Promise<Inspection> {
  const knownNamespaces = new Set(
    SCOPES.flatMap((scope) => WINDOWS.map((windowDays) => boardKey(scope, windowDays))),
  );
  ...
  for (let pageNumber = 0; pageNumber < MAX_PAGES && !publicComplete; pageNumber += 1) {
  ...
  for (let pageNumber = 0; pageNumber < MAX_PAGES && !namespacesComplete; pageNumber += 1) {
    const page = await doomerboard.paginateNamespaces(ctx, namespaceCursor, PAGE_SIZE);
  ...
  for (const namespace of namespaces) {
    ...
    let complete = false;
    for (let pageNumber = 0; pageNumber < MAX_PAGES && !complete; pageNumber += 1) {
      ...
      for (const row of page.page) {
        ...
        if (!expected || !namespaceIsValid || !namespaceMatches || matched.has(row.id)) {
          extraEntries += 1;
          repairs.push({
            id: row.id,
            namespace,
...
async function inspectDoomerboard(ctx: ActionCtx): Promise<Inspection> {
  for (let attempt = 0; attempt < MAX_STABLE_SCAN_ATTEMPTS; attempt += 1) {
    const before = await ctx.runQuery(internal.internal.doomerboardInvariantPage.version, {});
    const inspection = await inspectDoomerboardOnce(ctx);
    const after = await ctx.runQuery(internal.internal.doomerboardInvariantPage.version, {});
...
export const repair = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const request of inspection.repairs) {
      await ctx.runMutation(internal.internal.doomerboardInvariantPage.repairEntry, request);
    }
