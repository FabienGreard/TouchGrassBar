...
Run `bun run dev:desktop:fixture` to compare the native panel with fake data in the browser preview. The command prints the preview URL; append `?fixture=current&syncStatus=synced`. Both views use the same fixtures. This debug-only mode uses an isolated app namespace, disables provider reads and usage synchronization, skips onboarding, and keeps the panel open when focus moves to the browser. The panel fits its content up to 800 logical pixels or the screen work area. A shorter viewport can scroll without changing the content height used by the native resize observer.
...
The usage chart defaults to Combined and Today. Today uses recorded hourly buckets in UTC; the 7-day and 30-day views use Daily Usage Aggregates. The optional `usageHistory` projection in the native snapshot contains at most three provider scopes, 30 daily buckets and 24 hourly buckets per scope, and one model winner for each period. Rust builds this projection during refresh, before the revision comparison. Panel reads use only the cached projection. Provider disablement and UTC day changes clear obsolete projections. Model winners use the full local model counts for the selected period; private or unknown model names do not enter IPC.

Codex index version 11 stores independent response checkpoints and the dates of known response gaps. Parser version 25 counts each response once, including compaction, and does not repeat previous cumulative usage when a task resumes. It records accepted hourly and daily tokens in the same transaction. A smaller account bucket cannot erase validated local usage; source selection follows ADR 0016. Replay, file replacement, and fork re-accounting replace both projections. The index removes hour detail after its UTC day ends. Claude hour detail uses the existing deduplicated messages. Both adapters use the current pricing catalog. A chart does not distribute a provider-reported daily total over hours. Missing hours become zero only after a complete scan reconciles hourly and daily tokens. When the sums differ, the UI identifies the local hourly detail. The shared chart and controls also serve the landing page; synthetic records are confined to its demo and browser fixtures.
...
The scanner records nine reviewed Claude Code versions: `2.1.223`, `2.1.224`,
`2.1.236`, `2.1.241`, `2.1.258`, `2.1.259`, `2.1.260`, `2.1.261`, and `2.1.263`.
