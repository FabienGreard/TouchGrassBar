...
A record from another version is read with the same structural checks, keeps its
Observed Tokens, and leaves its Ranking Day partial and unpriced. Only a record
that is unreviewed and also carries an unreviewed usage shape withholds its
counters. [ADR 0020](adr/0020-audit-coding-provider-contracts-against-reviewed-snapshots.md)
