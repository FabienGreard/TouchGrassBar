...
The usage summary shows **Usage** as its section title. When recognized model evidence exists, the same line shows **Most used · Model name** as supporting context. TouchGrassBar ranks Codex and Claude model evidence from the current UTC Ranking Day, combines aliases under one catalog display name, and excludes disabled providers. The supporting context is `—` when today's model evidence is absent or the winning evidence has no recognized model. Yesterday's model and Today total do not carry into the new Ranking Day. The interface does not show the ranking period in this line.
...
A full provider observation is required to initialize a Quota Snapshot. A sparse provider notification may update an existing snapshot but cannot create one. A successfully observed Quota Lane is current for five minutes. A restored or old Quota Snapshot is stale until a full provider observation replaces it. At reset, the old lane leaves the active set, but its last-known allowance, remaining value, and reset time remain visible. A failed refresh or Mac wake preserves the last-known stale value rather than turning it into zero.

## Usage evidence and corrections

Observed Usage records three independent facts: its evidence basis is provider-reported or locally derived; its coverage is complete or partial; and current-day availability is current, stale, or unavailable. A period is mixed when its selected Ranking Days use both evidence bases. Complete means the selected evidence proves its declared provider scope and supported token categories through the observation time. Any known gap makes it partial. Partial Observed Tokens still contribute their known value to Token Score, with no imputation. Missing usage is unavailable, never zero or estimated. Completed historical days remain available with their recorded coverage rather than becoming stale with age.

Exactly one source owns each provider and Ranking Day. Provider-reported usage wins when it is present. Valid locally derived usage is the fallback when the provider has no bucket for that day. TouchGrassBar never sums a provider-account total with local tokens for the same day. If neither source has valid evidence, the day is unavailable.

Claude uses its provider total when that total is available for a Ranking Day. Local Claude evidence is the fallback and supplies private cost detail.

Codex uses an account daily bucket when that bucket is present. If the account response omits a Ranking Day, valid local rollout evidence supplies the locally derived fallback for that day. If the cache retains an older account bucket for an omitted day, local evidence replaces it in the display only when its count is greater and its last usage event is after the bucket's observation time. Otherwise, the saved account count remains selected. A returned account bucket takes priority again, including a lower correction or zero. An omitted bucket is not evidence of zero usage. If neither source has valid evidence, the day is unavailable. A multi-day Codex period can therefore contain provider-reported and locally derived days. Local model evidence also supplies private cost detail and can model an estimate for a provider total. Price coverage does not control token source selection. An unresolved inherited rollout makes the local fallback partial, but it does not discard proved local tokens.

Copied Codex history uses one trusted parent and the last safe parent snapshot at or before the fork. TouchGrassBar subtracts that snapshot per token category. Missing, ambiguous, cyclic, or unstable lineage contributes no local tokens and makes local coverage partial.
