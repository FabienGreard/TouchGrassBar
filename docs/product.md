...
Settings lists Codex and Claude with an enabled-by-default control. A disabled provider remains visible in registry order with unavailable Quota Lanes. TouchGrassBar does not start later refresh or probe work for it. Its Observed Usage and API-Equivalent Cost are excluded from Combined totals. Its local history is retained so a later re-enable can restore the last valid usage and cost immediately while the provider row shows that quota refresh is in progress. A re-enabled provider without history does not replace another enabled provider's valid Combined value with an indexing state.
