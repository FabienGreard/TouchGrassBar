...
Codex uses an account daily bucket when that bucket is present. If the account response omits a Ranking Day, valid local rollout evidence supplies the locally derived fallback for that day. An omitted bucket is not evidence of zero usage. If no valid local evidence exists, the day is unavailable. A multi-day Codex period can therefore contain provider-reported and locally derived days. Local model evidence also supplies private cost detail and can model an estimate for a provider total. Price coverage does not control token source selection. An unresolved inherited rollout makes the local fallback partial, but it does not discard proved local tokens.
