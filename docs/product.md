...
The usage summary shows one **Top model** label. TouchGrassBar ranks recognized Codex and Claude model evidence from the latest 30 UTC days, combines aliases under one catalog display name, and excludes disabled providers. It shows `—` when the winning evidence has no recognized model. The interface does not show the ranking period in this line.
