# Verify the Usage SQLite Schema Release
...
changing the stored activation time. Codex usage-index version 8 changes only
the account usage cache. It stores an observation time for each UTC day and a
separate successful-refresh time. This change lets a sparse provider response
keep omitted cached days without making them look newly observed.
...
2. Rehearse the Codex and sanitized read-model version 6 to version 7
   migrations and the Codex version 7 to version 8 account-cache migration on
   disposable copies.
...
and test results. Confirm database format 7, read model 7, Codex usage 8, and
...
completion field with the pending default. Confirm that the Codex version 7
source migrates only the account cache to version 8, keeps rollout-index rows
and metadata byte-for-byte, and assigns the prior refresh time to each
migrated account day.
...
The release that contains Codex usage-index version 8 and sanitized read-model
version 7 is published, and issue #73 contains passing count-only evidence for every
