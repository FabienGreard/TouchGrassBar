...
type NativeContractExport = {
  addTokenmaxxerContractVersion: number;
  addTokenmaxxerOutcomeSchema: JsonSchema;
  approvedPricingBasesByProvider: Record<"claude" | "codex", string[]>;
...
const addTokenmaxxerOutcomeSchema = contract.addTokenmaxxerOutcomeSchema;
...
const definitions = {
  ...(addTokenmaxxerOutcomeSchema.$defs ?? {}),
...
export const ADD_TOKENMAXXER_CONTRACT_VERSION = ${JSON.stringify(contract.addTokenmaxxerContractVersion)} as const;
export const APPROVED_PRICING_BASES_BY_PROVIDER = ${JSON.stringify(contract.approvedPricingBasesByProvider)} as const;
...
export const addTokenmaxxerOutcomeSchema = ${render(pinContractVersion(addTokenmaxxerOutcomeSchema, contract.addTokenmaxxerContractVersion))};
...
export type AddTokenmaxxerOutcome = z.infer<typeof addTokenmaxxerOutcomeSchema>;
