...
test("the branded recovery dialog sends the exact trimmed credentials", async () => {
  ...
  render(<RecoveryDialog onOpenChange={onOpenChange} onRecover={onRecover} open />);

  expect(screen.getByRole("heading", { name: "Recover from another Mac" })).toBeDefined();
...
test("missing credentials reach the generic recovery failure path", async () => {
  ...
  render(<RecoveryDialog onOpenChange={vi.fn()} onRecover={onRecover} open />);
  ...
  await waitFor(() => {
    ...
    expect(screen.getByRole("status").textContent).toContain("Profile recovery unavailable");
  });
});
